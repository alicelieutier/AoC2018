...
let show_map_with_carts carts matrix =
  ...
  let char_for_direction = function
    ...
    | North -> '^'
  in
  List.iter carts ~f:(fun {x; y; direction; _} ->
      map.(x).(y) <- char_for_direction direction ) ;
...
let show_carts carts =
  let show_cart {x; y; direction; _} =
    Format.printf "%d,%d going %s@." y x (string_of_direction direction)
  in
...
let move_one_cart matrix {x; y; direction; last_turn} =
  ...
  in
  {x= new_x; y= new_y; direction= new_direction; last_turn}
...
let move_once matrix cart_positions carts =
  let next_state (cart_positions, carts) cart =
    ...
    if IntPairSet.mem cart_positions (new_x, new_y) then Error (new_x, new_y)
    else
  ...
  let sorted_carts =
    List.sort
      ~compare:(fun {x= ax; y= ay; _} {x= bx; y= by; _} ->
        if ax = bx then ay - by else ax - bx )
      carts
  in
...
let move nb_of_times matrix cart_positions carts =
  ...
  in
  let result =
    List.fold_result (range 0 nb_of_times) ~f:aux ~init:(cart_positions, carts)
  in
  match result with
  | Ok _ -> Format.printf "No crash after %d seconds@." nb_of_times
  | Error (second, x, y) ->
      Format.printf "Crash after %d seconds: position %d,%d@." second y x
...
let move_once_safe matrix cart_positions carts =
  let next_state (cart_positions, carts, positions_to_remove) cart =
    ...
    if IntPairSet.mem positions_to_remove (old_x, old_y) then
      ( cart_positions
      , carts
      , IntPairSet.remove positions_to_remove (old_x, old_y) )
    else
      let new_cart = move_one_cart matrix cart in
      let {x= new_x; y= new_y; _} = new_cart in
      if IntPairSet.mem cart_positions (new_x, new_y) then
        ...
        let carts =
          List.filter carts ~f:(fun {x; y; _} -> (x, y) <> (new_x, new_y))
        in
        let positions_to_remove =
          if List.length carts = l1 then
            IntPairSet.add positions_to_remove (new_x, new_y)
          else positions_to_remove
        in
  ...
  let sorted_carts =
    List.sort
      ...
      carts
  in
  List.fold sorted_carts ~f:next_state
    ~init:(cart_positions, [], IntPairSet.empty)

let move_safe nb_of_times matrix cart_positions carts =
  let aux (cart_positions, carts) second =
    let cart_positions, carts, _ =
      move_once_safe matrix cart_positions carts
    in
  ...
  match result with
  | Ok (cp, carts) ->
      Format.printf "OK (%d carts left)@." (IntPairSet.length cp) ;
      carts
  | Error (second, cp, carts) ->
      Format.printf "%d cart left after %d seconds@." (IntPairSet.length cp)
        second ;
      show_cp cp ;
      carts
...
let () =
  ...
  Format.printf "dimensions of the map: width %d, height %d@."
    (Array.length matrix)
    (Array.length matrix.(0)) ;
  Format.printf "number of carts: %d@." (List.length carts) ;
  (* part 1 *)
  let cart_positions =
    IntPairSet.of_list (List.map carts ~f:(fun {x; y; _} -> (x, y)))
  in
