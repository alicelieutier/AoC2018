...
let parse_line line =
  ...
  (fun x y step_x step_y -> { x = x; y = y; step_x = step_x; step_y = step_y })

(* returns a new point, created by moving the starting point 'spoint' 'time' times *)
let move time { x = x; y = y; step_x = step_x; step_y = step_y } =
...
let position_in_image (width, _) (top_offset, left_offset) { x = x; y = y } =
  let pos_y, pos_x =  y - top_offset, x - left_offset in
  (pos_y * width + pos_x) 

let place_point_in_image dim (top_offset, left_offset) image point =
  Bigarray.Array1.set image (position_in_image dim (top_offset, left_offset) point) 0xff;
  ()

let create_image name (width, height) (top_offset, left_offset) image points =
  List.iter points ~f: (place_point_in_image (width, height) (top_offset, left_offset) image);
...
let create_image_at_time max_size original_points time =
  ...
  if width * height < max_size then
    ...
    let image = Bigarray.Array1.create Int8_unsigned Bigarray.c_layout (width * height) in
...
let () =
  part1 "day10_ocaml/input_test" 100; (* HI *)
  part1 "day10_ocaml/input" 1000; (* AHFGRKEE *)
