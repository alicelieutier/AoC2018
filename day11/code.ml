...
let cumulative_weights serial = 
  ...
  let rec aux (x, y) =
    ...
    | None -> 
      ...
      | x, y -> aux (x, (y - 1)) + aux ((x - 1), y) - aux ((x - 1), (y - 1)) + calculate_cell_level serial (x, y)
      in
...
let area_sum cs top_left bottom_right =
  ...
  | _, _ -> cs bottom_right - cs (tl_x - 1, br_y) - cs (br_x, tl_y - 1) + cs (tl_x - 1, tl_y - 1)
...
let find_max_power_any_size serial =
  ...
  in
  let max_sum, identifier = List.fold sizes_to_try ~f: aux ~init: (0, (1, 1, 300)) in
max_sum, identifier
