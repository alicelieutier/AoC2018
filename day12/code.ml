...
let match_rules rules =
  ...
  List.iter rules ~f:(fun (pattern, plant) ->
      Hashtbl.set h ~key:pattern ~data:plant ) ;
  let aux surrounding =
    match Hashtbl.find h surrounding with Some true -> '#' | _ -> '.'
  in
  aux
...
let test () =
  ...
  let new_state =
    List.fold operations
      ~f:(fun acc _ -> iterate mr acc)
      ~init:padded_initial_state
  in
...
let () =
  ...
  let new_state =
    List.fold operations
      ~f:(fun acc _ -> (* Format.printf "state: %s@." acc; *) iterate mr acc)
      ~init:padded_initial_state
  in
  Format.printf "state: %s@." new_state ;
  ...
  let final_result =
    result_200 + ((50000000000 - 200) * (result_202 - result_201))
  in
